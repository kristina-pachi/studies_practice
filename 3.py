...
def result():
    name = entry.get() # Получаем текст из поля ввода
    answers = [q1.get(), q2.get(), q3.get(), q4.get()]  # Все ответы
    ...
    if max_type == 'a':
        res = "Ты – булочка с маком, и пока одни в тебе души не чают, другие предпочитают обходить за километр. Да, с некоторыми тебе не по пути, зато те, кто смог «распробовать начинку», ни за что не останутся разочарованными."
    elif max_type == 'b':
        res = "Ты – булочка с изюмом. Скажем прямо – характер у тебя, судя по всему, не из простых, но тем ценнее становятся люди, которые смогли его раскусить и принять во всем многообразии. Впрочем, кто знает, где еще у тебя припрятана горстка изюма?"
    else:
        res = "Ты – «Московская» плюшка, а это значит, что тебя просто невозможно не любить. Идеальное комбо мягкого хлеба и сахарной сладости автоматически делают любой день лучше – как и ты своим появлением."
